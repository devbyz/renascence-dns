...
CLASSPATH=$CLASSPATH:$SERVER_HOME/src/main/resources
...
if [ "${env_access_file}" != '' ]; then
    ...
    JAVA_OPTS="${JAVA_OPTS} -Xms$HEAP_MEMORY -Xmx$HEAP_MEMORY -XX:MetaspaceSize=$PERM_MEMORY -XX:MaxMetaspaceSize=$PERM_MEMORY -Duser.dir=$SERVER_HOME -Dbase=$SERVER_HOME -Djava.io.tmpdir=$SERVER_HOME/tmp/run.log -Dlog4j.config=$SERVER_HOME/conf/log4j.properties"
